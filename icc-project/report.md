...
| B1 | stoppingDistance | 75.7445 | 40.0000 | 0.00 / 5 |
...
그러나 stoppingDistance는 72.2992 m에서 75.7445 m로 증가하였다. 이는 제동력을 보수적으로 제한한 결과이다. 실제 ABS는 wheel slip ratio를 직접 측정하여 최적 slip 부근에서 제동력을 유지하지만, 본 설계에서는 wheel slip ratio가 직접 입력으로 제공되지 않아 감속도 ax 기반으로 제동력을 완화하였다. 따라서 slip 안정성은 개선되었지만 제동거리는 증가하였다.
